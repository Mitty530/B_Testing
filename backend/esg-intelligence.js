...
// Root GET route for /api/esg-intelligence
router.get('/', (req, res) => {
  res.json({
    message: 'Borouge ESG Intelligence API is running',
    availableRoutes: [
...
console.log("the query received", query);
    // Validate input
    if (!query || typeof query !== 'string' || query.trim().length === 0) {
      return res.status(400).json({
        error: $`{query} : Query is required and must be a non-empty string`,
        timestamp: new Date().toISOString()
      });
    }
    ...
    if (articles.length < 1) {
      ...
      return res.status(404).json({
        error: `${query} : Insufficient relevant articles found for analysis`,
